...
### **Найти индекс**
Ваша задача — реализовать функцию, которая принимает отсортированный **массив** (`array`) и значение (`value`) и возвращает индекс элемента с данным значением. Временная сложность алгоритма должна быть **O(logN)**.

Например:

`findIndex([1, 2, 3], 1) => 0`

Напишите свой код в `src/st-find-index.js`.
